...
# TO GET ANSWER FROM BARD API ---------------------------
import aims
import getAnswer as ga
from code_writer import checkError, generateRequirements
code = True
description = True
# ans = ga.getDescription(aims.desAim)

# # Save the ans in output.txt
...
for aim in aims.aims:
    code = ga.isCode(aim)
    description = ga.isDescription(aim)
    if code:
        with open(f".\codes\pr{aims.aims.index(aim)}.py", "w") as f:
            print(f"pr{aims.aims.index(aim)}.py is being written")
            f.write(ga.getCode(aim))
            print(f"pr{aims.aims.index(aim)}.py is written", end=" - ")
            # Check for errors in code
            if checkError(f".\codes\pr{aims.aims.index(aim)}.py"):
                print("No errors in code")
            else:
                print("Error/s in code")
...
# paragraph.text = "Information and Security\t\t12002080601105"
# paragraph.style.font.size = aw.shared.Pt(13)
